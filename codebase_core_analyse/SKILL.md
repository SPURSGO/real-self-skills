...
- 文档格式使用 markdown 格式
- 文档生成的位置在当前工作区的doc目录下

## 重要规则

1. **始终使用可点击的引用**：[文件名.cpp:42](src/文件名.cpp#L42)
2. **数据类型必须精确**：不要只说"string"，要说"string（最大255字符，UTF-8编码）"
3. **记录所有字段**：请求/响应文档中绝不遗漏任何字段
4. **展示真实示例**：使用代码中的实际值或真实的示例
5. **追踪完整流程**：从开始到结束跟踪代码执行
6. **注明安全影响**：认证、授权、输入验证
7. **忽略样板代码**：专注业务逻辑，除非关键否则不关注日志/工具函数
